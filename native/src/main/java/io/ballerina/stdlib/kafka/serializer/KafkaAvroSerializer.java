...
public class KafkaAvroSerializer implements Serializer<Object> {
    ...
    public KafkaAvroSerializer(BString schemaRegistryUrl, BString avroSchema) {
        try {
            this.serializer = (schemaRegistryUrl != null && avroSchema != null)
                    ? ValueCreator.createObjectValue(getModule(), "KafkaAvroSerializer",
    ...
    public byte[] serialize(String topic, Object value) {
        ...
        record.put(StringUtils.fromString("name"), StringUtils.fromString("Hellow"));
        record.put(StringUtils.fromString("favourite_color"), StringUtils.fromString("Yellow"));
        BString kafkaTopic = StringUtils.fromString(topic);
        Object[] arguments = new Object[]{record};
        Object result = ModuleUtils.getEnvironment()
                .getRuntime().callMethod(this.serializer, "serialize", null, arguments);
        if (result instanceof BArray bArray) {
            return bArray.getByteArray();
        }
        return "dd".getBytes();
